...
<template>
  <main class="animationDemo">
    <section class="sectionDemo">
      <h2>CSS 클래스 기반 애니메이션</h2>
      <div class="logos">
        <button type="button" :class="{ animateWiggle: move }" @click="move = !move">
          <img :src="vueLogo" alt="Vue 로고" height="30" />
        </button>
        <button type="button" :class="{ animatePulse: rotate }" @click="rotate = !rotate">
    ...
    <section class="sectionDemo">
      <h2>상태 기반 애니메이션</h2>
      <div ref="animateBoxRef" class="animateBox" @pointermove="handleMove">
        <p>마우스를 움직여보세요.<br />{ x: {{ x }} }</p>
      </div>
    </section>
    <section class="sectionDemo">
      <h2>워치 &amp; 애니메이션</h2>
      <div class="animateWatchController">
        <label for="animateWatch">
          <abbr title="Automated Teller Machine">ATM</abbr>
          무통장입금 한도
        </label>
        <div class="animateWatchInput">
