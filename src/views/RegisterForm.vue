...
import type { Euid } from '@/plugins/euid';

const { checkEmail, checkPassword, delay } = inject('euid') as Euid;
...
<template>
  ...
  <form v-else class="form" novalidate @submit.prevent="handleSubmit" @reset.prevent="handleReset">
    <FormInput ref="nameRef" id="userName" label="이름" placeholder="이름" v-model="formData.name" />
    <FormInput
      ref="emailRef"
      type="email"
      id="userEmail"
